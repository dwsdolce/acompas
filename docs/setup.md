...
2. `node scripts/icons.mjs` — generates the icons for every target this host
   can build, so Xcode and Android Studio find theirs without a build first.
...
Every icon in the project comes from **`resources/artwork/logo.svg`**. The build
renders it to a 1024×1024 master at `resources/icon.png` and generates everything
else from that; the splash screens additionally use `background-gradient.png` as
their ground. Those two committed files are the only icon sources there are —
the master is generated like everything below it.

Nothing has to be refreshed by hand. `scripts/icons.mjs` runs from the build's
`beforeBuild` and `beforeDev` hooks, works out whether the icons still match the
artwork by hashing the sources, and regenerates only what the target being built
consumes — a macOS build has no use for `icon.ico` or the Linux set.

```bash
yarn icons              # every target this host can build, if you want it early
yarn icons ios android  # or just the named ones
```

No generated icon is committed. That was the bug this replaced: the set used to
be produced once by `postinstall`, so pulling a commit that changed the artwork
left every later build shipping the old mark with nothing to say so.
...
Generating the iOS set also runs `packaging/prepare-ios-assets.mjs`, which
flattens the alpha channel out of the app icon (App Store Connect rejects an icon
that has one) and copies the splash to the 1x and 2x names Xcode expects. It runs
on macOS, Windows and Linux, and needs ffmpeg — the same ffmpeg the audio uses. It
